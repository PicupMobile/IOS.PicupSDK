...
open class NetworkService {
    ...
    public func getSessionToken(_ securityCode: String , completion: ErrorClosure?) {
        request(query: .sessionToken(organizationCode: organizationCode, firebaseID: deviceToken, securityCode: securityCode)) { (sessionToken: SessionToken?, error: Error?) in
    ...
    public func registerClientDevice(phoneNumber: String, name: String = UIDevice.current.name, completion: GenericCallback<ClientDevice>?) {
        ...
        request(query: .registerClient(organizationCode: organizationCode, registrationID: deviceToken, phoneNumber: phoneNumber, deviceName: name, deviceGuid: deviceGuid), completion)
    }

    public func pullMessageData(status: String, completion: GenericCallback<MessageData>?) {
        request(query: .pullMessage(organizationCode: organizationCode, registrationID: deviceToken, status: status), completion)
    }
    ...
    public func sendCallbackData(messageFrom: String, fromDevice: String, toDevice: String, mode: String, status: String, completion: GenericCallback<GenericStatus>?) {
        request(query: .callbackData(organizationCode: organizationCode, registrationID: deviceToken, messageFrom: messageFrom, fromDevice: fromDevice, toDevice: toDevice, mode: mode, status: status), completion)
    }

    public func getCampaignByPhoneNumber(outgoingPhoneNumber: String, completion: GenericCallback<GenericStatus>?) {
        request(query: .campaignByPhoneNumber(organizationCode: organizationCode, registrationID: deviceToken, outgoingPhoneNumber: outgoingPhoneNumber), completion)
    }

    public func respondTopic(fromDevice: String, status: String, completion: GenericCallback<GenericStatus>?) {
        request(query: .respondTopic(organizationCode: organizationCode, registrationID: deviceToken, fromDevice: fromDevice, status: status), completion)
    }
    ...
    fileprivate enum Query {
        case sessionToken(organizationCode:String, firebaseID: String, securityCode: String)
        case registerClient(organizationCode:String, registrationID: String, phoneNumber: String, deviceName: String, deviceGuid: String)
        case pullMessage(organizationCode:String, registrationID: String, status: String)
        case callbackData(organizationCode:String, registrationID: String, messageFrom: String, fromDevice: String, toDevice: String, mode: String, status: String)
        case campaignByPhoneNumber(organizationCode:String, registrationID: String, outgoingPhoneNumber: String)
        case respondTopic(organizationCode:String, registrationID: String, fromDevice: String, status: String)
        ...
        fileprivate var params: Parameters {
            ...
            switch self {
            case let .sessionToken(organizationCode, firebaseID, securityCode):
                params = [
                    "id": firebaseID,
                    "code": securityCode,
                ]
                params["organizationCode"] = organizationCode
            case let .registerClient(organizationCode, registrationID, _, _, _),
                 let .pullMessage(organizationCode, registrationID, _),
                 let .callbackData(organizationCode, registrationID, _, _, _, _, _),
                 let .campaignByPhoneNumber(organizationCode, registrationID, _),
                 let .respondTopic(organizationCode, registrationID, _, _):
                params["registrationID"] = registrationID
                params["organizationCode"] = organizationCode
                
                switch self {
                case let .pullMessage(_, _, status),
                     let .callbackData(_, _, _, _, _, _, status),
                     let .respondTopic(_, _, _, status):
                    params["status"] = status
                default: break
                }

                switch self {
                case let .registerClient(_, _, phoneNumber, deviceName, deviceGuid):
                    params["phoneNumber"] = phoneNumber
                    params["deviceName"] = deviceName
                    params["deviceGuid"] = deviceGuid
                case let .callbackData(_, _, messageFrom, fromDevice, toDevice, mode, _):
                    params["messageFrom"] = messageFrom
                    params["fromDevice"] = fromDevice
                    params["toDevice"] = toDevice
                    params["mode"] = mode
                case let .campaignByPhoneNumber(_, _, outgoingPhoneNumber):
                    params["outgoingPhoneNumber"] = outgoingPhoneNumber
                case let .respondTopic(_, _, fromDevice, _):
                    params["fromDevice"] = fromDevice
                default: break
                }
            }
